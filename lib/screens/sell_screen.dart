...
class _SellScreenState extends State<SellScreen> {
  ...
  Future<void> _submitListing() async {
    ...
    try {
      _logError('Starting image upload...');

      final database = Provider.of<DatabaseService>(context, listen: false);

      // Upload images with timeout
      final imageUrls = await database
          .uploadCarImages(_imageFiles, user.uid)
          .timeout(const Duration(seconds: 30), onTimeout: () {
        throw Exception('Image upload timed out after 30 seconds');
      });

      _logError('Images uploaded successfully: ${imageUrls.length}');
      ...
      _logError('Timeout Error: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Upload took too long. Please check your connection'),
          backgroundColor: Colors.red,
        ),
      );
    } catch (e, stack) {
      ...
      if (mounted) {
        setState(() {
          _isUploading = false;
          _uploadStatus = 'Process completed';
        });
      }
    }
  }
