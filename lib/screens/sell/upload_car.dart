...
class _UploadCarScreenState extends State<UploadCarScreen> {
    ...
    try {
      ...
      if (pickedFiles != null && pickedFiles.isNotEmpty) {
        setState(() {
          ...
          if (remainingSlots > 0) {
            _images.addAll(pickedFiles.length > remainingSlots
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // Create temporary car ID
      final tempCarId = 'temp_${DateTime.now().millisecondsSinceEpoch}';

      // Upload images first
      final imageUrls =
          await storageService.uploadCarImages(tempCarId, _images);
